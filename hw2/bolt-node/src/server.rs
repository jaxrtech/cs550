    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                debug!("[server] received from client {:?}", result);
                ...
                if let Ok(req) = result {
                    if let RequestBody::Fetch(fetch) = req {
                        ...
                        debug!("[server] handler -> {:?}", result);
                        if let Ok(Some(response)) = result {
                            let mut socket_lock = socket_write.lock().await;
                            let send_result = response.write_to(socket_lock.deref_mut()).await;
                            drop(socket_lock);
                            debug!("[server] sent response! {:?}", send_result);
                        } else {
                            error!("[server] bad message from response handler: {:?}", result);
                        }
                    }
                } else {
                    debug!("[server] shutdown client handler");
                    break;
                }
            }
        });
...
fn handle_fetch(fetch: messages::FileFetchRequest, socket_ref: Arc<Mutex<OwnedWriteHalf>>, root_directory: PathBuf) -> task::JoinHandle<()> {
    tokio::spawn(async move {
        let fetch = fetch;
        let root_directory = root_directory;
        debug!("[server] root = '{}'", &root_directory.to_string_lossy());
